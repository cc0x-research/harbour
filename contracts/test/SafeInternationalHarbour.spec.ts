...
// Tests for the addSignature function
describe("addSignature", () => {
	async function deployFixture() {
		...
		// Create a transaction and enqueue it first
		...
		const safeTxHash = getSafeTransactionHash(safeAddress, chainId, safeTx);
		const signature1 = await signer1.signTypedData(
			{ chainId, verifyingContract: safeAddress },
			EIP712_SAFE_TX_TYPE,
			safeTx,
		);
		...
		return { deployer, alice, harbour, chainId, safeAddress, safeTx, safeTxHash, signer1 };
	}

	it("should successfully add a signature to an existing transaction", async () => {
		const { harbour, chainId, safeAddress, safeTx, safeTxHash } = await loadFixture(deployFixture);
		const signer2 = ethers.Wallet.createRandom();
		const signer2Address = signer2.address;
		...
		const listIndexBefore = await harbour.retrieveSignaturesCount(signer2Address, safeAddress, chainId, safeTx.nonce);
		expect(listIndexBefore).to.equal(0);

		await harbour.addSignature(safeAddress, chainId, safeTx.nonce, safeTxHash, signature2);

		const count = await harbour.retrieveSignaturesCount(signer2Address, safeAddress, chainId, safeTx.nonce);
		expect(count).to.equal(1);

		const [page] = await harbour.retrieveSignatures(signer2Address, safeAddress, chainId, safeTx.nonce, 0, 1);
		expect(page.length).to.equal(1);
		expect(page[0].txHash).to.equal(safeTxHash);

		const { r, vs } = toCompactSignature(signature2);
		expect(page[0].r).to.equal(r);
		expect(page[0].vs).to.equal(vs);
	});

	it("should revert with MissingTxDetails if the transaction doesn't exist", async () => {
		const { harbour, chainId, safeAddress, safeTx } = await loadFixture(deployFixture);
		const signer2 = ethers.Wallet.createRandom();
		const nonExistentTxHash = ethers.keccak256("0xdeadbeef");
		...
		await expect(
			harbour.addSignature(safeAddress, chainId, safeTx.nonce, nonExistentTxHash, signature2),
		).to.be.revertedWithCustomError(harbour, "MissingTxDetails");
	});

	it("should revert if attempting to add the same signature twice", async () => {
		const { harbour, chainId, safeAddress, safeTx, safeTxHash } = await loadFixture(deployFixture);
		const signer2 = ethers.Wallet.createRandom();
		const signer2Address = signer2.address;
		...
		await expect(harbour.addSignature(safeAddress, chainId, safeTx.nonce, safeTxHash, signature2))
			.to.be.revertedWithCustomError(harbour, "SignerAlreadySignedTransaction")
			.withArgs(signer2Address, safeTxHash);
	});

	it("should emit SignatureStored event with correct parameters", async () => {
		const { harbour, chainId, safeAddress, safeTx, safeTxHash } = await loadFixture(deployFixture);
		const signer2 = ethers.Wallet.createRandom();
		const signer2Address = signer2.address;
		...
		await expect(harbour.addSignature(safeAddress, chainId, safeTx.nonce, safeTxHash, signature2))
			.to.emit(harbour, "SignatureStored")
			.withArgs(signer2Address, safeAddress, safeTxHash, chainId, safeTx.nonce, 0);
	});

	it("should store multiple different signatures for the same transaction", async () => {
		const { harbour, chainId, safeAddress, safeTx, safeTxHash } = await loadFixture(deployFixture);
		...
		for (let i = 0; i < signers.length; i++) {
			...
			await expect(harbour.addSignature(safeAddress, chainId, safeTx.nonce, safeTxHash, signature))
				.to.emit(harbour, "SignatureStored")
				.withArgs(signers[i].address, safeAddress, safeTxHash, chainId, safeTx.nonce, 0);

			// Check signature was stored correctly
			const count = await harbour.retrieveSignaturesCount(signers[i].address, safeAddress, chainId, safeTx.nonce);
			expect(count).to.equal(1);
		}
	});

	it("should revert with InvalidECDSASignatureLength if signature length is not 65 bytes", async () => {
		const { harbour, chainId, safeAddress, safeTx, safeTxHash } = await loadFixture(deployFixture);

		await expect(
			harbour.addSignature(
				safeAddress,
				chainId,
				safeTx.nonce,
				safeTxHash,
	...
	it("should revert with InvalidSignature if ecrecover yields zero address", async () => {
		const { harbour, chainId, safeAddress, safeTx, safeTxHash } = await loadFixture(deployFixture);
		const invalidSig = `0x${"00".repeat(65)}`;

		await expect(
			harbour.addSignature(safeAddress, chainId, safeTx.nonce, safeTxHash, invalidSig),
		).to.be.revertedWithCustomError(harbour, "InvalidSignature");
	});

	it("should revert with InvalidSignatureSValue if S value is not from lower half of curve", async () => {
		const { harbour, chainId, safeAddress, safeTx, safeTxHash } = await loadFixture(deployFixture);
		const signer = ethers.Wallet.createRandom();

		// Create a valid signature first
		const validSignature = await signer.signTypedData(
			{ chainId, verifyingContract: safeAddress },
			EIP712_SAFE_TX_TYPE,
			safeTx,
		);

		// Modify the S value to be in the upper half of the curve
		const sigBytes = ethers.getBytes(validSignature);
		...
		// If s is already in upper half, this is fine for our test
		// If s is in lower half, we need to move it to upper half
		let sUpper = sBN;
		if (sBN <= secp256k1N / 2n) {
			sUpper = secp256k1N - sBN;
		}

		const sUpperHex = ethers.toBeHex(sUpper, 32);
		const invalidSignature = ethers.concat([r, sUpperHex, ethers.toBeHex(v, 1)]);

		await expect(
			harbour.addSignature(safeAddress, chainId, safeTx.nonce, safeTxHash, invalidSignature),
		).to.be.revertedWithCustomError(harbour, "InvalidSignatureSValue");
	});

	it("should work in a complete workflow with enqueueTransaction and addSignature", async () => {
		const { harbour, chainId, safeAddress } = await loadFixture(deployFixture);

		// Create a new transaction
		const nonce = 99n;
		const originalSigner = ethers.Wallet.createRandom();
		const additionalSigners = [ethers.Wallet.createRandom(), ethers.Wallet.createRandom()];
		...
		const newSafeTxHash = getSafeTransactionHash(safeAddress, chainId, newSafeTx);

		// First signer creates the transaction
		const originalSignature = await originalSigner.signTypedData(
			{ chainId, verifyingContract: safeAddress },
			EIP712_SAFE_TX_TYPE,
			newSafeTx,
		);
		...
		for (const signer of additionalSigners) {
			...
			await harbour.addSignature(safeAddress, chainId, newSafeTx.nonce, newSafeTxHash, signature);
		}

		// Verify all signatures were stored
		expect(await harbour.retrieveSignaturesCount(originalSigner.address, safeAddress, chainId, nonce)).to.equal(1);
		expect(await harbour.retrieveSignaturesCount(additionalSigners[0].address, safeAddress, chainId, nonce)).to.equal(
			1,
		);
		expect(await harbour.retrieveSignaturesCount(additionalSigners[1].address, safeAddress, chainId, nonce)).to.equal(
			1,
		);

		// Verify transaction details were stored correctly
		const storedTx = await harbour.retrieveTransaction(newSafeTxHash);
		expect(storedTx.to).to.equal(newSafeTx.to);
		expect(storedTx.value).to.equal(newSafeTx.value);
		expect(storedTx.data).to.equal(newSafeTx.data);
	});

	it("should properly isolate signatures by chain ID", async () => {
		const { harbour, safeAddress, safeTx, safeTxHash } = await loadFixture(deployFixture);
		const differentChainId = 999n; // Different from the one used in fixture
		const signer = ethers.Wallet.createRandom();

		// Sign the same transaction for a different chain
		const signatureForDifferentChain = await signer.signTypedData(
			{ chainId: differentChainId, verifyingContract: safeAddress },
			EIP712_SAFE_TX_TYPE,
			safeTx,
		);

		// Create the transaction on the different chain first
		// We need to calculate the expected hash for this chain
		const differentChainTxHash = getSafeTransactionHash(safeAddress, differentChainId, safeTx);

		// First we need to enqueue the transaction for the different chain
		await harbour.enqueueTransaction(
			safeAddress,
			differentChainId,
			safeTx.nonce,
			...
			safeTx.gasToken,
			safeTx.refundReceiver,
			signatureForDifferentChain,
		);

		// Now add another signature for the different chain
		const anotherSigner = ethers.Wallet.createRandom();
		const anotherSignature = await anotherSigner.signTypedData(
			{ chainId: differentChainId, verifyingContract: safeAddress },
			EIP712_SAFE_TX_TYPE,
			safeTx,
		);

		await harbour.addSignature(safeAddress, differentChainId, safeTx.nonce, differentChainTxHash, anotherSignature);

		// Verify signatures are correctly isolated
		expect(
			await harbour.retrieveSignaturesCount(anotherSigner.address, safeAddress, differentChainId, safeTx.nonce),
		).to.equal(1);
		expect(
			await harbour.retrieveSignaturesCount(anotherSigner.address, safeAddress, safeTx.nonce, safeTx.nonce),
		).to.equal(0);
	});
});
